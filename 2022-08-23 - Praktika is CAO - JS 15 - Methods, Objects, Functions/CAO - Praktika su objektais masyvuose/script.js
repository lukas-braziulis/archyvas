...
console.log(data.reduce((acc, person) => {
...
console.log(data.reduce((acc, person) => person.gender.toLowerCase() === 'male' ? acc + 1 : acc , 0));
...
console.log(data.filter((person) => person.shirt_size.toLocaleLowerCase() === 'xs' || person.shirt_size.toLocaleLowerCase() === 's').sort((a, b) => {
    return a.first_name > b.first_name ? 1 : -1; // konkreciai tikrins ir rykiuos pagal vardus. Reikalingos "? 1 : -1" kodo dalis, kad rusiuotu.
}));

// trumpesne versija.
console.log(
    data.filter((person) => person.shirt_size.toLocaleLowerCase() === 'xs' || person.shirt_size.toLocaleLowerCase() === 's').sort((a, b) => a.first_name > b.first_name ? 1 : -1)
    )

// 4. Pakoreguokite trečią pratimą, kad masyve matytųsi tik id, vardas bei marškinių dydis.

console.log(
    data.filter((person) => person.shirt_size.toLocaleLowerCase() === 'xs' || person.shirt_size.toLocaleLowerCase() === 's')
    .sort((a, b) => a.first_name > b.first_name ? 1 : -1)
    .map((person) => {
        return {
            id: person.id,
            vardas: person.first_name,
            marskiniuDydis: person.shirt_size
        }
    }))
    // viską vis tiek būtų geriau įdėti į funkciją, nes funkciją gali apibūdinti kas yra viduje. Taip pat tu gali ją panaudoti dar kartą. IR tik iš funkcijos tu gali pasiimti tą Datą 

    // PVZ.:

    function filteredIdNameShirtSize(array) {
        return array.filter((person) => person.shirt_size.toLocaleLowerCase() === 'xs' || person.shirt_size.toLocaleLowerCase() === 's')
        .sort((a, b) => a.first_name > b.first_name ? 1 : -1)
        .map((person) => {
            return {
                id: person.id,
    ...
    console.log(filteredIdNameShirtSize(data)); // iškviečiant funkciją vietoj f-jos parametro 'array' bus pateiktas 'data' masyvas, kurį panaudojai iškviečiant funkciją